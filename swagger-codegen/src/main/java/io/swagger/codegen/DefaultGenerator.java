...
public class DefaultGenerator extends AbstractGenerator implements Generator {
    ...
    private String getPort() {
    	String portValue = StringUtils.EMPTY;
        if (!StringUtils.isEmpty(swagger.getHost()) && StringUtils.contains(":", swagger.getHost())) {
            portValue =  StringUtils.split(swagger.getHost(),':')[1];
        } 
        return portValue;
    }
    ...
    private void configureGeneratorProperties() {
        ...
        //Get Swagger Host instead of localhost in template
        if (!StringUtils.isEmpty(swagger.getHost())) {
        	 baseHost = swagger.getHost();
        }else{
        	baseHost = "localhost";
        }
        if (!StringUtils.isEmpty(swagger.getHost())) {
    ...
    private void generateApis(List<File> files, List<Object> allOperations, List<Object> allModels) {
        ...
        for (String tag : paths.keySet()) {
            try {
                ...
                operation.put("baseHost", baseHost);
                operation.put("basePort", basePort);
                ...
                //Generate files for each operation having post/put data
                for (CodegenOperation op : ops) 
                {
                	if(!StringUtils.isEmpty(op.requestExample))
                	{
                	  for (String templateName : config.apiTemplateFiles().keySet()) {
                          String filename = config.apiFilename(templateName, op.nickname);
                ...
                //Generate api files other than json 
                for (String templateName : config.apiTemplateFiles().keySet()) {
    ...
    private Map<String, Object> buildSupportFileBundle(List<Object> allOperations, List<Object> allModels) {
        ...
        bundle.put("baseHost", baseHost);
        bundle.put("basePort", basePort);
